...
- **Figura 1** — custo de rekey vs N, com foco na **latência end-to-end de rede** (`rekey_e2e_ms`) e, complementarmente, mensagens/operações/CPU/bytes/ACKs.
- **Figura 2** — perda de pacotes ao longo do tempo, alinhada ao instante do revoke (**métrica complementar** de disponibilidade).
...
- **`_b64e()` / `_b64d()`** — codifica/decodifica bytes em Base64 para transportar ciphertext no JSON.
...
- **`_decrypt()`** — contraparte do `_encrypt()`: decifra `nonce||ciphertext`.
- **`emit_event()`** — escreve **uma linha por evento** no `protocol_latency.csv`, incluindo `rekey_msgs`, `crypto_ops`, `rekey_ms`, `rekey_e2e_ms`, `rekey_bytes_total`, `acks_received`, `acks_expected`.
...
- **`GroupState`** (classe) — mantém membros, revogados, época (epoch) e o estado criptográfico.
  - **`_current_group_key()`** — retorna a chave de grupo atual (no LKH, a raiz da árvore).
  - **`update_member_endpoint()`** — registra/atualiza endpoint `IP:porta` de cada membro para recebimento de push.
  - **`_build_rekey_push_targets()`** — monta payloads cifrados por membro para distribuição real do rekey.
  ...
  - **`register()`** — registro inicial de um membro (**não** dispara rekey) e retorna `kek_b64` do membro.
  - **`join()`** — entrada de membro (dispara rekey de adição) e retorna `kek_b64`.
...
- **`AuthTCPHandler`** (classe) — handler TCP que recebe requisições JSON e chama `register/join/leave/revoke/status`.
  - **`_send_push_and_wait_ack()`** — envia `rekey_push` para um membro e aguarda ACK (timeout/retry configuráveis).
  - **`_dispatch_rekey_push()`** — faz push do rekey para todos os membros alvo, coleta ACKs e mede `rekey_e2e_ms` + `rekey_bytes_total`.
...
- **`send_request_with_stats()`** — envia requisição e retorna também os bytes transmitidos (base de `rekey_bytes_total`).
- **`MemberRekeyPushHandler`** (classe) — listener TCP do membro que recebe `rekey_push`, decifra com KEK local, instala nova chave e responde ACK.
- **`start_member_rekey_listener()`** — inicia o listener de push no membro (`--member-port`, padrão 9100).
- **`new_member_context()`** — inicializa contexto criptográfico local (`member_kek`, `group_key`, `epoch`).

**Modos de execução (escolhidos por `--role`/`--event`):**

- **`run_auth_server()`** — sobe o servidor TCP 9000 e mantém o `GroupState`.
- **`run_member()`** — inicia listener de rekey, registra o drone, armazena `kek_b64` recebida e entra em **heartbeat**.
- **`run_join()`** — inicia listener de rekey, executa **join** e armazena `kek_b64`.
...
- **`start_auth_server()`** — sobe o auth-server com `--rekey-scheme` e `--member-port`.
- **`start_group_member()`** — inicia um drone como membro (repassando `--member-port`).
- **`request_join()` / `request_leave()`** — disparam join/leave (também com `--member-port`).
- **`revoke_member()`** — dispara a **revogação** de um drone pela autoridade; é síncrono (sem `&`), bloqueando até finalizar push+ACKs do rekey.
...
- **`consolidate_figure1()`** — lê os `protocol_latency.csv`, filtra eventos **`revoke`** e escreve uma linha por evento em `figure1_rekey_cost.csv` (`rekey_scheme, group_size, run_id, rekey_msgs, crypto_ops, rekey_ms, rekey_e2e_ms, rekey_bytes_total, acks_received, acks_expected`).
...
- **`plot_figures.py`** — lê `figure1_rekey_cost.csv` e `figure2_packet_loss.csv` e gera os PNGs (`plot_figure1`, `plot_figure2`, `_savefig`), aceitando `--metric rekey_msgs|crypto_ops|rekey_ms|rekey_e2e_ms|rekey_bytes_total`.
...
1. O **auth-server** (`group_auth.py` dentro do contêiner `auth1`), ao processar o `revoke`, mede custo lógico/CPU (`rekey_msgs`, `crypto_ops`, `rekey_ms`), **transmite** a nova chave cifrada para cada membro via push TCP, coleta ACKs e grava `rekey_e2e_ms`, `rekey_bytes_total`, `acks_received`, `acks_expected` em `/tmp/drone-logs/protocol_latency.csv`.
...
rekey_ms, rekey_e2e_ms, rekey_bytes_total, acks_received, acks_expected, extra
```

A linha relevante para a Figura 1 é a do evento `revoke`, que carrega também `rekey_e2e_ms`, `rekey_bytes_total`, `acks_received` e `acks_expected`.
...
- **naive:** `rekey_msgs ~ N-1` e `rekey_e2e_ms` tende a crescer com N (push sequencial).
- **lkh:** `rekey_msgs ~ O(log N)` e `rekey_e2e_ms` tende a crescer bem menos.

Esse contraste — visível na Figura 1 (incluindo `--metric rekey_e2e_ms`) — é o **resultado central** que valida a vantagem do LKH sobre o re-chaveamento ingênuo em grupos de drones. A Figura 2 complementa mostrando a perda de pacotes em torno do revoke.
